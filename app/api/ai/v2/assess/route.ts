...
const assessmentSchema = z.object({
  damage: z
    .array(
      z.object({
        ...
        confidence: z.number().min(0).max(1).describe("Confidence score of the detection (0-1)"),
      }),
    )
  ...
  vinDecodedSpecs: z.record(z.any()).optional().describe("Decoded VIN specifications if available"),
})
...
export async function POST(req: NextRequest) {
  ...
  try {
    const { imageUrls, vinNumber, vinDecodedSpecs } = await req.json()
    ...
    let prompt = `
      ...
      Provide a confidence score for each detection (0-1).
    ...
    if (vinDecodedSpecs) {
      prompt += `
        ...
        ${JSON.stringify(vinDecodedSpecs, null, 2)}
        Use these specifications to inform your assessment, especially for parts and labor estimates.
      `
    }
    ...
    console.error("AI assessment v2 failed:", error)
    status = "error"
    errorMessage = error.message || "Internal server error during AI assessment v2."
    return NextResponse.json({ error: errorMessage }, { status: 500 })
  } finally {
    trackApiCall("ai_assessment_v2", Date.now() - startTime, status, errorMessage)
  }
}
